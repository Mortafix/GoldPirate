...
class TorrentDownloads:
	def __init__(self):
		self.url = 'https://torrentdownloads.unblockit.win'
	...
	def _search_torrents(self,query,sort=None):
		...
		return reduce(lambda x,y:x+y,[bs(requests.get('{}{}{}'.format(self.url,url_attach,sub('@@',str(i+1),self.page)),headers=self.user_agent).text, 'html.parser').findAll('div',{'class':'grey_bar3'})[6:-1] for i in range(2)],[])
	...
	def _get_category(self,cateogry_id):
		categories = {4:'Movies',5:'Music',8:'Television',3:'Games',7:'Software',1:'Anime',2:'E-Books',9:'Other'}
