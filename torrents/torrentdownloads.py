...
class TorrentDownloads:
	...
	def _search_torrents(self,query,pages,sort=None):
		...
		return reduce(lambda x,y:x+y,[bs(requests.get('{}{}{}'.format(self.url,url_attach,sub('@@',str(i+1),self.page)),headers=self.user_agent).text, 'html.parser').findAll('div',{'class':'grey_bar3'})[6:-1] for i in range(pages)],[])
