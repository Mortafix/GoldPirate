...
class CorsaroNero:
	def __init__(self):
		...
		self.sort_type = {'age':'data','size':'size','seed':'seeds','leech':'peers'}
		self.page = '&page=@@'
	...
	def _search_torrents(self,query,pages,sort=None):
		...
		return reduce(lambda x,y:x+y,[bs(requests.get('{}{}{}'.format(self.url,url_attach,sub('@@',str(i),self.page)),headers=self.user_agent).text, 'html.parser').findAll('td')[18:-3] for i in range(pages)],[])
	...
	def build_list(self,query,pages,sort=None):
		...
		for torrent in singles:
			...
			link_page = sub(self.url,'',[a.get('href') for a in torrent[1].findAll('a')][0])
			size = torrent[2].text
			time,seed,leech = ['0' if x == 'n/a' else x for x in [t.text for t in torrent[3:]]]
			torrents.append(['CorsaroNero',name,link_page,sub(r'(\d\d)$',r'20\1',time),category,sub(r'\d+$','',size),int(sub(',','',seed)),int(sub(',','',leech))])
		return torrents
